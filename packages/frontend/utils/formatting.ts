...
export function trim(value :string, decimals : number){
  if (!DECIMAL_REGEX.test(value)) {
    throw new TypeError('Invalid input, decimal string expected.')
  }
  
  const num = value;
  const decimalSeparator = num.indexOf('.');
   
  if(decimalSeparator<0)
    return num;
  
  const decimalLength = num.length - decimalSeparator +1;
  return decimalLength < decimals ? num : num.substr(0,decimalSeparator + decimals );  
}
